...
Sub-bab ini menjelaskan ruang lingkup dan batasan dari sistem yang dikembangkan agar pembahasan dalam penelitian ini tetap terarah dan terfokus. Ruang lingkup menggambarkan cakupan fungsionalitas sistem yang dibangun, sedangkan batasan sistem menjelaskan hal-hal yang tidak termasuk dalam pengembangan sistem ini. Keduanya ditetapkan berdasarkan hasil analisis kebutuhan yang telah dilakukan sebelumnya.

### 1.4.1 Ruang Lingkup Sistem

Sistem yang dikembangkan dalam penelitian ini dirancang dengan menerapkan arsitektur client-server modern yang memisahkan tanggung jawab antara antarmuka pengguna (frontend) dan backend. Sistem ini terdiri dari tiga modul utama yang saling terintegrasi, yaitu:
...
1. **Ruang Lingkup Operasional:** Sistem difokuskan pada layanan pengiriman dalam kota yang dikelola oleh armada internal toko. Sistem ini berdiri sendiri dan tidak terintegrasi dengan API ekspedisi pihak ketiga (seperti JNE atau J&T).
...
Bab ini menjelaskan identifikasi dan analisis masalah yang dihadapi toko modern dalam mengelola operasional bisnisnya, meliputi studi kasus, analisis kebutuhan, dan perumusan kebutuhan sistem.
