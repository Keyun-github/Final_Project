...
class _ChatRoomPageState extends State<ChatRoomPage> {
  ...
  void _setupSocket() {
    _ws.connectToChat();
    _ws.joinOrderChat(widget.orderId, widget.userType, widget.driverId);

    _messageSubscription = _ws.messageStream.listen((message) {
  ...
  Future<void> _loadMessages() async {
    ...
    try {
      final response = await ApiService().get('/chat/conversation/${widget.conversationId}/messages');
  ...
  void _sendMessage() {
    ...
    if (!_ws.isChatConnected) {
      debugPrint('[ChatRoom] Chat socket not connected');
    ...
    final senderId = widget.userType == 'driver' ? widget.driverId : widget.customerId;
  ...
  @override
  void dispose() {
    _ws.leaveOrderChat(widget.orderId);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          Icon(
            _ws.isChatConnected ? Icons.wifi : Icons.wifi_off,
            color: _ws.isChatConnected ? Colors.white : Colors.red,
            size: 20,
          ),
